...
def get_orientations_possible(block: np.ndarray) -> List[List[dict]]:
    ...
    poss2 = [
        ...
        [{'k': 3, 'axes': (2, 1)}],
        [{'k': 2, 'axes': (0, 1)}],
        [{'k': 2, 'axes': (0, 1)}, {'k': 1, 'axes': (2, 1)}],
        [{'k': 2, 'axes': (0, 1)}, {'k': 2, 'axes': (2, 1)}],
        [{'k': 2, 'axes': (0, 1)}, {'k': 3, 'axes': (2, 1)}]
    ]
    ...
    if height == depth == width:
        return poss
    elif (height != depth != width) & (height != width):
        return poss1
    elif (height == depth) or (height == width) or (depth == width):
        return poss2
...
def carve_sculpture_from_density_block(shape: np.ndarray, block: np.ndarray) -> np.ndarray:
    ...
    >>> bool(shape_1.shape==marble_block_1.shape)
    True
    >>> shape_1.dtype
    dtype('int8')
    >>> marble_block_1.dtype
    dtype('float16')
    """
    ...
    carve=shape*block
    if shape.size != block.size or shape.shape != block.shape:
        raise ValueError
    return carve
...
def is_stable(sculpture: np.ndarray) -> bool:
    ...
    >>> marble_block_1 = np.load(file='data/marble_block_1.npy')
    >>> shape_1 = np.load(file='data/shape_1.npy')
    >>> carve1 =shape_1*marble_block_1
    >>> is_stable(carve1)
    'Stable'
    >>> marble_block_4 = np.load(file='data/marble_block_4.npy')
    >>> carve4 =shape_1*marble_block_4
    >>> is_stable(carve4)
    'Stable'

    """
    v = sculpture[-1]
    points = np.argwhere(v > 0)
    ch1 = ConvexHull(points=points, incremental=True)
    orginal_area = ch1.area
    sculpture[np.isnan(sculpture)] = 0
    center_mass = center_of_mass(sculpture)[1:3]
    center_mass = np.asarray(center_mass)
    center_mass = center_mass.reshape(1, 2)
    points = np.append(points, center_mass, axis=0)
    ch2 = ConvexHull(points=points, incremental=True)
    new_area = ch2.area
    if new_area == orginal_area:
        return "Stable"
    else:
        return "Unstable"
...
def analyze_sculptures(block_filenames: list, shape_filenames: list):
    ...
    for i in shape_filenames:
        print('Shape File :shape_'+str(shape_count)+'.npy')
        shape_count +=1
        block_count=1
        for j in block_filenames:
            print('{:<3}Block File :block_'.format('') + str(block_count) + '.npy')
            block_count += 1
            orgin_value = np.nanmean(carve_sculpture_from_density_block(i, j))
            stable=is_stable(carve_sculpture_from_density_block(i, j))
            print("{:<7}Rotation:None {:<28}mean density : {:.2f} {:<40}".format('','',orgin_value,stable))
            for k in get_orientations_possible(j):
                s=k[0]['axes'][0]
                if len(k)==2:
                    new_jj = np.rot90(j, k=k[0]['k'], axes=k[0]['axes'])
                    new_j = np.rot90(new_jj, k=k[1]['k'], axes=k[1]['axes'])
                    value = np.nanmean(carve_sculpture_from_density_block(i, new_j))
                    stable = is_stable(carve_sculpture_from_density_block(i, new_j))
                    print("{:<7}Rotation:{:<4} axis {} to {:<1},{:<3} axis {} to {} mean density : {:.2f} {}".format('',k[0]['k'] * 90, k[0]['axes'][0],
                                                                                                                  k[0]['axes'][1],k[1]['k'] * 90,k[1]['axes'][0],k[1]['axes'][1], value,stable))
                else:
                    new_j=np.rot90(j,k=k[0]['k'],axes=k[0]['axes'])
                    value = np.nanmean(carve_sculpture_from_density_block(i, new_j))
                    stable=is_stable(carve_sculpture_from_density_block(i, new_j))
                    print("{:<7}Rotation:{:<4} axis {} to {:<17} mean density : {:.2f} {}".format('',k[0]['k'] * 90, k[0]['axes'][0],
                                                                                   k[0]['axes'][1], value,stable))
...
if __name__ == '__main__':
    ...
    block_list = [marble_block_1, marble_block_2, marble_block_3, marble_block_4, marble_block_5]
