...
def get_orientations_possible(block: np.ndarray) -> List[List[dict]]:
    ...
    >>> marble_block_1 = np.load(file='marble_block_1.npy')
...
def carve_sculpture_from_density_block(shape: np.ndarray, block: np.ndarray) -> np.ndarray:
    ...
    >>> carve_sculpture_from_density_block(np.array([[2,3],[3,4]]), np.array([2,3]))
    ValueError
    """
...
def is_stable(sculpture: np.ndarray) -> bool:
    ...
    # TODO: Add a few good, working Doctests
    points = []
    for i in range(0, np.shape(sculpture)[1]):
        for j in range(0, np.shape(sculpture)[2]):
            if np.isnan(sculpture[-1][i][j]):
                points = points
            else:
                points.append([i, j])
...
def analyze_sculptures(block_filenames: list, shape_filenames: list):
    ...
    # TODO: Add a few good, working Doctests
    marble = {}
    shape = np.load(shape_filenames)
    block = np.load(block_filenames)
    sculpture = carve_sculpture_from_density_block(shape, block)
    if is_stable(sculpture):
        stable = "Stable"
    else:
        stable = "Unstable"
    marble["None"] = [np.nanmean(sculpture.astype("float32")), stable]
    for a in get_orientations_possible(block):
        for v in a:
            block = np.rot90(block, k=v['k'], axes=v['axes'])
        sculpture = carve_sculpture_from_density_block(shape, block)
        if is_stable(sculpture):
            stable = "Stable"
        else:
            stable = "Unstable"
        marble[a] = [np.nanmean(sculpture.astype("float32")), stable]
    sorted_marble = sorted(sorted(marble.items(), key=lambda y: y[1][1]), key=lambda y: y[1][0], reverse=True)
    for x in sorted_marble:
        if x[0] == 'None':
            print("Rotation: {:} Mean density:{:.2f} {}".format(x[0], x[1][0], x[1][1]))
        elif len(x[0]) == 1:
            print("Rotation: {} {} to {} Mean density:{:.2f} {}".
                  format(x[0]['k'] * 90, x[0]['axes'][0], x[0]['axes'][1], x[1][0], x[1][1]))
        else:
            print("Rotation: {} {} to {}, {} {} to {} Mean density:{:.2f} {}".
                  format(x[0][0]['k'] * 90, x[0][0]['axes'][0], x[0][0]['axes'][1],
                         x[0][1]['k'] * 90, x[0][1]['axes'][0], x[0][1]['axes'][1], x[1][0], x[1][1]))
...
if __name__ == '__main__':
    ...
    analyze_sculptures('data/marble_block_1.npy', 'data/shape_1.npy')
